...
select
  ...
  null::text as access_descriptor,
...
left join (select
  ...
  timepoint,
  id
from {{ ref('inc_brainpower_src_bp_age_event_latency') }}) as age_data
...
left join (select
  ...
  timepoint,
  id
from {{ ref('inc_brainpower_src_bp_age_event_latency') }}) as age_data2
